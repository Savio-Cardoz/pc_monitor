...
void recv_serial_data(const char *input_str, size_t data_len)
{
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        ssd1306_display_text(&dev, 0, "C_U  G_T  G_U", 16, false);

        sprintf(disp_data_buf, "%s ", parameter_array[1]);
        sprintf(disp_data_buf + strlen(disp_data_buf), "%s ", parameter_array[2]);
        sprintf(disp_data_buf + strlen(disp_data_buf), "%s ", parameter_array[3]);

        ssd1306_display_text(&dev, 1, disp_data_buf, strlen(disp_data_buf), false);

        ssd1306_display_text(&dev, 3, "Up time:", 8, false);
        // ssd1306_display_text(&dev, 1, parameter_array[2], strlen(parameter_array[2]), false);

        // ssd1306_display_text(&dev, 2, parameter_array[3], strlen(parameter_array[3]), false);

        ssd1306_display_text(&dev, 4, parameter_array[4], strlen(parameter_array[4]), false);
        ssd1306_display_text(&dev, 6, parameter_array[5], strlen(parameter_array[5]), false);
