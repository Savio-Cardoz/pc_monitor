...
extern void example_lvgl_demo_ui(lv_disp_t *disp, RingbufHandle_t ring_buffer);
...
void recv_serial_data(const char *input_str, size_t data_len)
{
    ...
    // Send an item
    UBaseType_t res = xRingbufferSend(buf_handle, data, sizeof(data), pdMS_TO_TICKS(1000));
    if (res != pdTRUE)
    {
